...
def block_entropy(sequence: list[str], block_size: int, bias_correction: bool = False) -> float:
    ...
    # Create L-grams
    blocks = tuple("".join(sequence[i : i + block_size]) for i in range(n - block_size + 1))
...
def entropy_rate(
    sequence: list[str], max_block_size: int, method: str = "incremental", min_block_size: int = 2
) -> float:
    """
...
def emc(sequence: list[str], max_block_size: int = 10, return_curve: bool = False) -> tuple:
    """
    ...
    if len(excess_curve) >= 4:
        ...
        warnings.warn("Insufficient block sizes for reliable EMC. Results may be unreliable.", UserWarning)
...
def detect_saturation_point(sequence: list[str], max_k: int, method: str = "adaptive") -> dict:
    """
...
def compute_adaptive_max_k(sequence: list[str], default_max_k: int = 6, min_samples_per_kgram: int = 5) -> int:
    """
...
def aggregate_saturation_results(saturation_results: list[dict]) -> dict:
    ...
    return {
        ...
        "confidence_counts": {"high": high_conf_count, "medium": medium_conf_count, "low": low_conf_count},
...
def _detect_saturation_from_cache(H_cache: dict[int, float], max_k: int, method: str = "adaptive") -> dict:
    ...
    return {"k_star": 2, "confidence": "low", "method_used": method, "increments": increments, "H_values": H_values}


def compute_entropy_metrics_ensemble(
    sequences: list[list[str]],
    max_block_size: int | None = None,
    min_sequence_length: int = 10,
    saturation_method: str = "hybrid",
) -> dict:
    """
    ...
    if not valid_sequences:
        warnings.warn(
            f"No sequences meet minimum length requirement ({min_sequence_length}). Returning default values.",
            UserWarning,
        )
        return {
            "block_entropy_k2": [],
